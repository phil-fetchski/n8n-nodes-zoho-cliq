...
This file provides guidance to AI coding agents working with code in this repository. It is included in the public repo so that anyone forking or branching this project can use AI-assisted development effectively from day one.
...
This project uses the `@n8n/node-cli` ESLint config which enforces n8n community node conventions:
...
- `strict: false` in `package.json` allows extending the ESLint config
