...
import { CHAT_MUTE_AGENT_DETAILS_LINK } from '../../helpers/linkConstants';
...
const muteRequiredScope = getRequiredScopeForOperation('chat', 'mute');
...
const properties: INodeProperties[] = [
	{
		...
		required: true,
		description:
			'Required Zoho Cliq chat ID for the conversation you want to mute. This is not a channel ID. Chat IDs may be all-numeric for some direct/private chats or use a `CT_...` style in other chat contexts.',
	...
	{
		displayName:
			'Mute Chat Docs: <a href="https://www.zoho.com/cliq/help/restapi/v2/#mute-chat" target="_blank" rel="noopener noreferrer">Open Zoho REST API Reference</a> REQUIRED SCOPES: <code>ZohoCliq.Chats.UPDATE</code>, <code>ZohoCliq.Chats.READ</code>',
		name: 'muteChatDocsNotice',
		type: 'notice',
		default: '',
	},
	{
		displayName: `Zoho Cliq Chat/Mute Chat as AI Tool Setup Guide: <a href="${CHAT_MUTE_AGENT_DETAILS_LINK}" target="_blank" rel="noopener noreferrer">Open Tool Setup Guide</a>`,
		name: 'muteChatAiToolGuideNotice',
		type: 'notice',
		default: '',
	},
];
...
export async function execute(
	...
	for (let i = 0; i < items.length; i++) {
		...
		try {
			checkRequiredScope(this, grantedScopes, muteRequiredScope, i);
			...
			const response = (await zohoCliqApiRequest.call(
				this,
				'POST',
				`/api/v2/chats/${encodeURIComponent(sanitizedId)}/mute`,
			)) as IDataObject;
			...
			if (
				pushChatRecoverableError(this, returnData, i, 'mute', error, {
					contextFields: requestedChatId?.length ? { chat_id: requestedChatId } : undefined,
					messageMappings: [
						{
