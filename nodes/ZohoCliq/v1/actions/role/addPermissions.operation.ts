/**
 * Add Role Permissions operation
 */
...
const requiredScope = getRequiredScopeForOperation('role', 'addPermissions');

const properties: INodeProperties[] = [
	{
		...roleIdLocator,
		description: 'The unique role ID to add permissions for',
	},
	{
	...
	{
		displayName: `Add Role Permissions Docs: <a href="https://www.zoho.com/cliq/help/restapi/v2/#update-role" target="_blank" rel="noopener noreferrer">Open Zoho REST API Reference</a> REQUIRED SCOPES: <code>${requiredScope}</code>`,
		name: 'addRolePermissionsDocsNotice',
		type: 'notice',
		default: '',
	},
	{
		displayName: ROLE_HELPER_OPERATION_NOT_RECOMMENDED_AS_AI_TOOL_NOTICE,
		name: 'addRolePermissionsAiToolGuideNotice',
		type: 'notice',
		default: '',
	},
];
...
export async function execute(
	...
	for (let i = 0; i < items.length; i++) {
		...
		try {
			...
			const list = collectSelectedPermissions(this, i, 'enabled');

			if (list.length === 0) {
				throw new NodeOperationError(this.getNode(), 'Select at least one permission to add.', {
					itemIndex: i,
				});
			}
			...
			if (
				pushRoleRecoverableError(this, returnData, i, 'addPermissions', error, {
					contextFields: requestedRoleId ? { role_id: requestedRoleId } : undefined,
					messageMappings: [
						{
							match: (normalizedMessage) =>
								normalizedMessage.includes('select at least one permission to add'),
							reason: 'NO_PERMISSIONS_SELECTED',
							hint: 'Select one or more supported structured permission entries to enable for the role.',
						},
						{
							match: (normalizedMessage) =>
