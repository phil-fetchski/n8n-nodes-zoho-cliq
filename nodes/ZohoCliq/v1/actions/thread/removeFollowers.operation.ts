/**
 * Remove Thread Followers operation
 * Removes users from following a thread
 */
...
import { THREAD_REMOVE_FOLLOWERS_AGENT_DETAILS_LINK } from '../../helpers/linkConstants';
...
const requiredScope = getRequiredScopeForOperation('thread', 'removeFollowers');
...
const properties: INodeProperties[] = [
	...
	},
	{
		...
		description:
			'Comma-separated list of user IDs to remove from followers. Whitespace around commas is ignored.',
	},
	{
		...
		description:
			"Whether to return workflow-friendly success metadata for this minimal remove-followers response. Disable to return Cliq's standard response.",
	},
	{
		displayName: `Remove Followers Docs: <a href="https://www.zoho.com/cliq/help/restapi/v2/#update-followers" target="_blank" rel="noopener noreferrer">Open Zoho REST API Reference</a> REQUIRED SCOPES: <code>${requiredScope}</code>`,
		name: 'removeThreadFollowersDocsNotice',
		type: 'notice',
		default: '',
	},
	{
		displayName: `Zoho Cliq Thread/Remove Followers as AI Tool Setup Guide: <a href="${THREAD_REMOVE_FOLLOWERS_AGENT_DETAILS_LINK}" target="_blank" rel="noopener noreferrer">Open Tool Setup Guide</a>`,
		name: 'removeThreadFollowersAiToolGuideNotice',
		type: 'notice',
		default: '',
	},
];
		...
		try {
			...
			const missingUserIds =
				...
				Array.isArray((error as Record<string, unknown>).zohoCliqMissingUserIds)
					...
					: undefined;
			if (
				pushThreadRecoverableError(this, returnData, i, 'removeFollowers', error, {
					contextFields: {
					...
					fallbackMessage: 'Unable to remove followers from the thread in Zoho Cliq.',
					messageMappings: [
						{
						...
						},
						{
							...
							hint: 'Use the exact thread chat ID returned by List Threads for Channel or Get Main Message before removing followers.',
						},
						{
							match: (normalizedMessage) =>
