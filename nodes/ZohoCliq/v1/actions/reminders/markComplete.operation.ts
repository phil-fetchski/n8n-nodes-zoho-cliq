/**
 * Mark Reminder Complete operation
 */

import type { IExecuteFunctions, INodeExecutionData, INodeProperties } from 'n8n-workflow';

import { REMINDERS_MARK_COMPLETE_AGENT_DETAILS_LINK } from '../../helpers/linkConstants';
...
const requiredScope = getRequiredScopeForOperation('reminders', 'markComplete');
...
const properties: INodeProperties[] = [
	...
	{
		displayName: `Mark Complete Docs: <a href="https://www.zoho.com/cliq/help/restapi/v2/#mark_reminder_complete" target="_blank" rel="noopener noreferrer">Open Zoho REST API Reference</a> REQUIRED SCOPES: <code>${requiredScope}</code>`,
		name: 'markReminderCompleteDocsNotice',
		type: 'notice',
		default: '',
	},
	{
		displayName: `Zoho Cliq Reminders/Mark Complete as AI Tool Setup Guide: <a href="${REMINDERS_MARK_COMPLETE_AGENT_DETAILS_LINK}" target="_blank" rel="noopener noreferrer">Open Tool Setup Guide</a>`,
		name: 'markReminderCompleteAiToolGuideNotice',
		type: 'notice',
		default: '',
	},
];
	...
	for (let i = 0; i < items.length; i++) {
		...
		try {
			...
			const response = await zohoCliqApiRequest.call(
				this,
				'PUT',
				`/api/v2/reminders/${encodeURIComponent(sanitizedReminderId)}/complete`,
			);
			...
			if (
				pushRemindersRecoverableError(this, returnData, i, 'markComplete', error, {
					...
					messageMappings: [
						...
						},
						{
							...
							hint: 'Use mark complete only for reminders in the Mine category.',
						},
					],
				})
			) {
