/**
 * Create Cliq Database row operation
 */
...
import { DATABASE_CREATE_AGENT_DETAILS_LINK } from '../../helpers/linkConstants';
...
const properties: INodeProperties[] = [
	...
	},
	{
		...
		name: 'createRecordValuesRaw',
		type: 'json',
		default: '{}',
		required: true,
		description:
			'Using JSON object for the record values. Supports object input or stringified JSON object.',
		displayOptions: {
			show: {
				createInputMode: ['raw'],
			},
	...
	{
		displayName:
			'Create Record Docs: <a href="https://www.zoho.com/cliq/help/restapi/v2/#insert_record" target="_blank" rel="noopener noreferrer">Open Zoho REST API Reference</a> REQUIRED SCOPES: <code>ZohoCliq.StorageData.CREATE</code>',
		name: 'createDatabaseRecordDocsNotice',
		type: 'notice',
		default: '',
	},
	{
		displayName: `Zoho Cliq Database/Create Record as AI Tool Setup Guide: <a href="${DATABASE_CREATE_AGENT_DETAILS_LINK}" target="_blank" rel="noopener noreferrer">Open Tool Setup Guide</a>`,
...
export async function execute(
	...
	for (let i = 0; i < items.length; i++) {
		...
		try {
			...
			const inputMode = validateDatabaseInputMode(
				this,
				this.getNodeParameter('createInputMode', i),
				i,
			);
			...
			if (inputMode === 'structured') {
				...
				const rawValues = this.getNodeParameter('createRecordValuesRaw', i, {}) as
					| IDataObject
					| string;
			...
			if (
				pushDatabaseRecoverableError(this, returnData, i, 'create', error, {
