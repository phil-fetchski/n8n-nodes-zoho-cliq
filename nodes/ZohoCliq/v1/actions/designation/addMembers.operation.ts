/**
 * Add Designation Members operation
 */

import type { IExecuteFunctions, INodeExecutionData, INodeProperties } from 'n8n-workflow';
import { NodeOperationError } from 'n8n-workflow';

import { DESIGNATION_ADD_MEMBERS_AGENT_DETAILS_LINK } from '../../helpers/linkConstants';
...
const properties: INodeProperties[] = [
	{
		...designationIdLocator,
		description: 'The unique designation ID to add members to.',
	},
	{
		...
		required: true,
		description:
			'User IDs to add to this designation. Accepts either a JSON array of canonical Zoho Cliq user ID strings or a comma-separated list for manual entry. AI Tool setups should use the JSON array form. Maximum 100 user IDs per request.',
	...
	{
		displayName: `Add Designation Members Docs: <a href="https://www.zoho.com/cliq/help/restapi/v2/#add-designation-members" target="_blank" rel="noopener noreferrer">Open Zoho REST API Reference</a> REQUIRED SCOPES: <code>${requiredScope}</code>`,
		name: 'addDesignationMembersDocsNotice',
		type: 'notice',
		default: '',
	},
	{
		displayName: `Zoho Cliq Designation/Add Designation Members as AI Tool Setup Guide: <a href="${DESIGNATION_ADD_MEMBERS_AGENT_DETAILS_LINK}" target="_blank" rel="noopener noreferrer">Open Tool Setup Guide</a>`,
		name: 'addDesignationMembersAiToolGuideNotice',
		type: 'notice',
		default: '',
	},
];
...
export async function execute(
	...
	for (let i = 0; i < items.length; i++) {
		...
		try {
			...
			if (
				pushDesignationRecoverableError(this, returnData, i, 'addMembers', error, {
					contextFields: {
					...
					messageMappings: [
						{
							...
							hint: 'Use the exact Zoho Cliq designation ID for the designation you want to add members to.',
						},
						{
							match: (normalizedMessage) =>
			...
			rethrowDesignationApiError(this, error, i, 'Add designation members', {
