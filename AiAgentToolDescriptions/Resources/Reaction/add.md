...
Use this guide to configure **Add Reaction** for AI Tool mode in n8n.

This operation adds one emoji reaction to one existing Zoho Cliq message.
...
```txt
Add one emoji reaction in Zoho Cliq to one existing message using `chat_id`, `message_id`, and `emoji_code`. Use the exact chat/message identifiers for the existing message. For `emoji_code`, send either a real Unicode emoji such as 👍 or a known Zoho Cliq shortcode such as `:smile:`.
...
- Plain text description:
```txt
Required Zoho Cliq message ID to react to inside the selected chat. Use the exact `id` returned by Get Messages or Retrieve Message, or the exact `message_id` returned by Post Message.
```
- Suggested `$fromAI()`:
```txt
{{ $fromAI('message_id', 'Required Zoho Cliq message ID to react to inside the selected chat. Use the exact `id` returned by Get Messages or Retrieve Message, or the exact `message_id` returned by Post Message.', 'string') }}
```
...
- Suggested `$fromAI()`:
```txt
{{ $fromAI('emoji_code', 'Required emoji to add as the reaction. Send either a real Unicode emoji such as 👍 or a known Zoho Cliq shortcode such as `:smile:`. Do not leave blank.', 'string') }}
```
