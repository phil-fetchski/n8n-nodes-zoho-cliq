...
Use this guide to configure **Update Event** for AI Agent Tool mode in n8n.

This operation updates one event in Zoho Cliq.

- Set **Input Mode** to `Using JSON`.
- Use `Calendar ID` as a separate required top-level field. Use `Event Updates (JSON)` for the agent-controlled update payload, not the structured Title, Time, Timezone, Type, attendee, reminder, or attachment fields.
...
```txt
Update one event in Zoho Cliq using `event_id`, `calendar_id`, and a raw JSON event-updates object. Required JSON fields are title, start_time, end_time, and timezone. Optional JSON fields include type, attendees, location, description, reminders, and attachments. Successful responses always include `updated`, `success`, `operation`, `event_id`, and `calendar_id` metadata, plus the most useful event fields. The nested `organizer.name` and `organizer.email` fields are flattened to `organizer_name` and `organizer_email`. Note: `meeting_link` and `meeting_details` are not included in Simplified output. If you need those fields, use Raw mode or include them in Selected Fields.
...
Include the matching IANA timezone in the `timezone` key of the event updates JSON, for example `America/New_York` for `-04:00` during Eastern Daylight Time. If the user's preferred timezone is not already known from context, infer the most likely IANA timezone from the user's locale, location, or stated offset and provide that timezone explicitly.
...
```txt
Update one event in Zoho Cliq using `event_id`, `calendar_id`, and a raw JSON event-updates object. Required JSON fields are title, start_time, end_time, and timezone. Optional JSON fields include type, attendees, location, description, reminders, and attachments. Successful responses always include `updated`, `success`, `operation`, `event_id`, and `calendar_id` metadata, plus only the configured Output Fields.

IMPORTANT: Calendar Events are meant to record events that are to happen in the future, not events that have already occurred. DO NOT EDIT A CALENDAR EVENT WITH PAST DATED TIMESTAMPS OR A MADE UP DATE, EVER.
...
```txt
Update one event in Zoho Cliq using `event_id`, `calendar_id`, and a raw JSON event-updates object. Required JSON fields are title, start_time, end_time, and timezone. Optional JSON fields include type, attendees, location, description, reminders, and attachments. Successful responses always include `updated`, `success`, `operation`, `event_id`, and `calendar_id` metadata, plus the full Zoho Cliq event response with all fields. For follow-up calls, reuse the top-level `event_id` and `calendar_id` metadata fields directly.
  ...
  "data": [
    {
      ...
      "start_time": 1738938600000,
      "end_time": 1738942200000,
      "meeting_link": "https://cliq.glencadia.com/meeting/1CI1MFX2BSUW",
      "meeting_details": {...}
    }
  ]
}
...
```txt
Required Zoho Cliq event ID to update. Use the exact event ID returned by a previous create, list, or get-details response.
```
- Suggested `$fromAI()`:
```txt
{{ $fromAI('event_id', 'Required Zoho Cliq event ID to update. Use the exact event ID returned by a previous create, list, or get-details response.', 'string') }}
```
...
- Plain text description:
```txt
Required exact Zoho Cliq calendar ID that owns the event being updated. Use the exact calendar_id returned by a previous create, list, or get-details response.
```
- Suggested `$fromAI()`:
```txt
{{ $fromAI('calendar_id', 'Required exact Zoho Cliq calendar ID that owns the event being updated. Use the exact calendar_id returned by a previous create, list, or get-details response.', 'string') }}
```

---

### Event Updates (JSON) (Required When Input Mode = Using JSON)

- Plain text description:
```txt
Required top-level Zoho Cliq event-update JSON object. This tool accepts either a stringified JSON object or a literal JSON object. Required keys are title, start_time, end_time, and timezone. Optional keys are type, attendees, location, description, reminders, and attachments. Do not include calendar_id here; use the separate required Calendar ID field instead. Always send start_time and end_time as ISO 8601 datetime strings with an explicit UTC offset, for example `2026-03-20T15:00:00-04:00`. Also provide the matching IANA timezone such as `America/New_York`. If the user's preferred timezone is unknown, infer the most likely IANA timezone from the user's locale, location, or stated offset and provide that timezone explicitly. type ENUM: ["normal_event", "event_management", "audio_conference", "video_conference"]. If type is omitted, the node preserves the existing event type automatically. If reminders are provided, use reminder objects with type `email` and a positive whole-number minutes value. Default reminder type to `email` for agent use. Note: For Update, attendees must be an array of objects with email and optional status ENUM: ["accepted", "declined", "tentative", "yet_to_respond"]. This differs from Create Event, which takes attendees as plain email strings. For attachments, use an array of objects with id values. Use the fileId returned by the Upload Event Attachment tool as the id value. This node still requires title, start_time, end_time, and timezone on update; for the remaining optional fields, omitting the field preserves the current value instead of explicitly clearing it. Unsafe keys such as __proto__, constructor, and prototype are rejected.
```
- Suggested `$fromAI()`:
```txt
{{ $fromAI('event_updates', 'Required top-level Zoho Cliq event-update JSON object. This tool accepts either a stringified JSON object or a literal JSON object. Required keys are title, start_time, end_time, and timezone. Optional keys are type, attendees, location, description, reminders, and attachments. Do not include calendar_id here; use the separate required Calendar ID field instead. Always send start_time and end_time as ISO 8601 datetime strings with an explicit UTC offset, for example 2026-03-20T15:00:00-04:00. Also provide the matching IANA timezone such as America/New_York. If the user\'s preferred timezone is unknown, infer the most likely IANA timezone from the user\'s locale, location, or stated offset and provide that timezone explicitly. type ENUM: [\"normal_event\", \"event_management\", \"audio_conference\", \"video_conference\"]. If type is omitted, the node preserves the existing event type automatically. If reminders are provided, use reminder objects with type `email` and a positive whole-number minutes value. Default reminder type to `email` for agent use. Note: For Update, attendees must be an array of objects with email and optional status ENUM: [\"accepted\", \"declined\", \"tentative\", \"yet_to_respond\"]. This differs from Create Event, which takes attendees as plain email strings. For attachments, use an array of objects with id values. Use the fileId returned by the Upload Event Attachment tool as the id value. This node still requires title, start_time, end_time, and timezone on update; for the remaining optional fields, omitting the field preserves the current value instead of explicitly clearing it. Do not include unsafe or unknown keys. Keys such as __proto__, constructor, and prototype are rejected. Only send the documented supported fields.', 'string') }}
```
...
- Plain text description:
```txt
Whether to return a simplified version of the response instead of the raw data. When enabled (default), the Simplify Mode selector controls the output shape. When disabled, the full raw API response is returned. Note: updated, success, operation, event_id, and calendar_id metadata are always included in the response regardless of this setting.
```
- Suggested `$fromAI()`:
```txt
{{ $fromAI('simplify', 'Whether to simplify the response. Set to true (default) for simplified output or false for the full raw API response. Note: updated, success, operation, event_id, and calendar_id metadata are always included regardless of this setting.', 'boolean', 'true') }}
```
