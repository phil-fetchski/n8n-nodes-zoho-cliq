...
Use this guide to configure **Get Department** for AI Agent Tool mode in n8n.

This operation retrieves one department in Zoho Cliq.
...
```txt
Get full details for one department in Zoho Cliq using department_id. Returns a flat object with the most useful fields, including the IDs you need for follow-up update, delete, add-members, get-members, or remove-members calls. Reuse id, parent_department_id, and lead_id directly.

For example:
{
  "id": "1901318000001071015",
  "name": "Marketing",
...
```txt
Get full details for one department in Zoho Cliq using department_id. Returns only the configured Output Fields. id is always included. Reuse id for follow-up update, delete, add-members, get-members, or remove-members calls.
...
```txt
Get full details for one department in Zoho Cliq using department_id. Returns the department object under data, including the IDs you need for follow-up update, delete, add-members, get-members, or remove-members calls. Reuse data.id, data.parent_department_id, and data.lead_zuid directly.
...
```txt
Required department_id in Zoho Cliq to retrieve. Use the exact department ID string, not a department name. Example: 1901318000001071015.
```
- Suggested `$fromAI()`:
```txt
{{ $fromAI('department_id', 'Required department ID in Zoho Cliq to retrieve. Use the exact department ID string, not a department name. Example: 1901318000001071015.', 'string') }}
```
