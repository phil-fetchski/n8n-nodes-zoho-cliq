# RemoteWork: Get Remote Work Status

Use this guide to configure **Get Remote Work Status** for AI Agent Tool mode in n8n.

This operation retrieves the authenticated user's current remote work status in Zoho Cliq.
...
```txt
Get the authenticated user's current remote work status in Zoho Cliq. This operation takes no additional input fields. Successful responses return a flat object with the most useful remote-work fields. Reuse `id` or `email_id` with later user, role, or profile-management steps. Nested objects like `department`, `designation`, and `status` are flattened to `department_name`, `designation_name`, and `status_message`.
...
```txt
Get the authenticated user's current remote work status in Zoho Cliq. This operation takes no additional input fields. Successful responses return only the configured Output Fields. `id` is always included. Reuse `id` or `email_id` with later user, role, or profile-management steps.
...
```txt
Get the authenticated user's current remote work status in Zoho Cliq. This operation takes no additional input fields. Successful responses return the full remote-work profile with attendance fields. Reuse `id`, `email_id`, or `zuid` with later user, role, or profile-management steps. Reuse `department.id` or `designation.id` for department and designation workflows.
...
- Plain text description:
```txt
Fields to include in the output when Simplify Mode is set to Selected Fields. The record ID is always included. Available fields: id, display_name, email_id, first_name, last_name, full_name, name, employee_id, checkin_status, checkin_time, checkin_status_text, checkin_allowed, duration, location, live_feed_status, status, department, designation, reportingto, recent_departments, timezone, country, language, mobile, zoid, organization_id, image_url, work_location, type, timeoffset, zuid, iamuid, organisation_members_count.
```
- Suggested `$fromAI()`:
```txt
{{ $fromAI('simplifyFields', 'Fields to include in the output. Available: id, display_name, email_id, first_name, last_name, full_name, name, employee_id, checkin_status, checkin_time, checkin_status_text, checkin_allowed, duration, location, live_feed_status, status, department, designation, reportingto, recent_departments, timezone, country, language, mobile, zoid, organization_id, image_url, work_location, type, timeoffset, zuid, iamuid, organisation_members_count. Return a JSON array of field names (e.g. ["id","checkin_status"]) or a comma-separated string (e.g. "id,checkin_status"). Both formats are accepted.', 'string', '["id"]') }}
```
