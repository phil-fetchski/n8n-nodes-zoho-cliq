# RemoteWork: Check Out

Use this guide to configure **Check Out** for AI Agent Tool mode in n8n.

This operation checks the authenticated user out of remote work in Zoho Cliq.
...
```txt
Check the authenticated user out of remote work in Zoho Cliq. This operation takes no additional input fields. Successful responses always include `success` and `operation` metadata, plus the simplified remote-work check-out fields. Drops the deeply nested `user_status_preference` object. Valid success responses can return a smaller subset of fields depending on Zoho's response.
...
```txt
Check the authenticated user out of remote work in Zoho Cliq. This operation takes no additional input fields. Successful responses always include `success` and `operation` metadata, plus only the configured Output Fields. `checkin_status` is always included.
...
```txt
Check the authenticated user out of remote work in Zoho Cliq. This operation takes no additional input fields. Successful responses always include `success` and `operation` metadata, plus the full Zoho Cliq remote-work check-out fields. Valid success responses can return a smaller subset such as checkin_allowed, checkin_status, and checkin_status_text, and may also include fields such as checkin_time, live_feed_status, user_status_preference, location, and duration.
