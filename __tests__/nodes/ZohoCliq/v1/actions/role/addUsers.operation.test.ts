...
import * as addUsers from '../../../../../../nodes/ZohoCliq/v1/actions/role/addUsers.operation';
import * as transport from '../../../../../../nodes/ZohoCliq/v1/transport';

jest.mock('../../../../../../nodes/ZohoCliq/v1/transport');

describe('ZohoCliq - Role - Add Users Operation', () => {
	...
	it('should add users to role successfully with enhanced output', async () => {
		...
		const result = await addUsers.execute.call(
			mockExecuteFunctions,
			items,
		...
		expect(mockZohoCliqApiRequest).toHaveBeenCalledWith('POST', '/api/v2/profiles/role_123/users', {
		...
		expect(result[0].json).toMatchObject({
			...
			operation: 'addUsers',
			role_id: 'role_123',
			added_user_ids: ['62913657', '63569660'],
			count: 2,
			status: 'success',
		});
	});
	...
	it('should return raw API response when enhanced output is disabled', async () => {
		const items: INodeExecutionData[] = [{ json: {} }];
		const apiResponse = { status: 'success', added: true };
	...
	it('should throw error for missing OAuth scope', async () => {
		...
		(mockExecuteFunctions.getNodeParameter as jest.Mock).mockImplementation((name: string) => {
			...
			if (name === 'operation') return 'addUsers';
			return undefined;
		});

		let thrownError: unknown;
		try {
			await addUsers.execute.call(mockExecuteFunctions, items, '');
		} catch (error) {
			thrownError = error;
		}
